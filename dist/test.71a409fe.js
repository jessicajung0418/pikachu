...
// 为了防止清除默认样式影响到btns，就在*前加.skin，表示只清除皮卡丘部分的默认样式(因为不加的话你会发现btns会先变大，再变小。)
...
// 把一大串影响浏览的代码放到一个文件里面（导出），然后再导入进来。 这种操作就叫：模块化。 ~~~~~~~~~
    ...
    bindEvents: function bindEvents() {
        for (var key in player.events) {
            if (player.events.hasOwnProperty(key)) {
                // 防御性编程，只遍历是自己拥有的属性，避免继承的属性造成影响
                var value = player.events[key]; // value的值是'pause'/ 'play' / 'slow'..
...
//# sourceMappingURL=/test.71a409fe.map